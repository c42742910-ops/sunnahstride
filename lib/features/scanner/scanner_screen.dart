...
class _ScannerState extends ConsumerState<ScannerScreen>
  ...
  void _scan(String barcode) {
    ...
    final product = kProductsDB.firstWhere((p) => p.barcode == barcode,
        orElse: () => ScanResult(
          ...
          notes:   isAr ? 'لا توجد بيانات' : 'No data found'));
  ...
  void _showLimitDialog(bool isAr) {
    showDialog(context: context, builder: (_) => AlertDialog(
      title: Text(isAr ? 'وصلت الحد اليومي' : 'Daily Limit Reached',
          style: const TextStyle(fontFamily: 'Cairo')),
      content: Text(isAr ? 'استخدمت ١٠ ماسحات اليوم.\nترقّ للبريميوم للمزيد.' : 'You\'ve used 10 scans today.\nUpgrade for unlimited.',
          style: const TextStyle(fontFamily: 'Cairo')),
      actions: [
        TextButton(onPressed: () => Navigator.pop(context),
            child: Text(isAr ? 'إغلاق' : 'Close', style: const TextStyle(fontFamily: 'Cairo'))),
        ElevatedButton(onPressed: () { Navigator.pop(context); context.push('/paywall'); },
            child: Text(isAr ? '⭐ ترقية' : '⭐ Upgrade', style: const TextStyle(fontFamily: 'Cairo'))),
      ],
    ));
  }
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        title: Text(t('الماسح الذكي 📷', 'Smart Scanner 📷')),
        actions: [
          GestureDetector(
      ...
      body: ListView(padding: const EdgeInsets.all(14), children: [
          ...
          child: Container(
            ...
            child: Row(children: [
              ...
              Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                Row(children: [
                  Text(t('تحليل الطعام بـ AI 🤖', 'AI Food Analyzer 🤖'),
                ...
                Text(
                  t('صوّر أي طعام ← سعرات + بروتين + حكم حلال فوراً',
                    'Photo any food ← Calories + Protein + Halal status instantly'),
        ...
        Row(children: [
          ...
          Padding(
            ...
            child: Text(t('أو امسح باركود', 'or scan barcode'),
        ...
        Row(children: [
          Expanded(child: TextField(
            ...
            decoration: InputDecoration(
              hintText: t('أدخل الباركود يدوياً...', 'Enter barcode manually...'),
        ...
        // ── Demo products ─────────────────────────────────
        Text(t('جرّب هذه المنتجات:', 'Try these products:'),
        ...
        ElevatedButton(
          ...
          child: Text(t('📷 مسح عشوائي', '📷 Random Scan'),
            ...
            child: Container(
              ...
              child: Row(children: [
                ...
                Expanded(child: Column(
                  ...
                  children: [
                    Text(p.name, style: const TextStyle(fontFamily: 'Cairo', fontSize: 10,
                        fontWeight: FontWeight.w700), maxLines: 1, overflow: TextOverflow.ellipsis),
                    Text(p.barcode, style: TextStyle(fontFamily: 'Cairo', fontSize: 9, color: muted)),
                  ]),
                ),
              ]),
            ),
        ...
        child: Column(children: [
          Row(mainAxisAlignment: MainAxisAlignment.center, children: [
            ...
            Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              Text(label, style: TextStyle(fontFamily: 'Cairo', fontWeight: FontWeight.w900,
                  fontSize: 20, color: col)),
              Text(r.name, style: const TextStyle(fontFamily: 'Cairo', fontSize: 14,
                  fontWeight: FontWeight.w600)),
              if (r.brand != null && r.brand!.isNotEmpty)
                Text(r.brand!, style: TextStyle(fontFamily: 'Cairo', fontSize: 11, color: muted)),
            ]),
          ]),
          ...
          Row(children: [
            Expanded(child: OutlinedButton.icon(
              ...
              label: Text(t('مسح آخر', 'Scan Again'), style: const TextStyle(fontFamily: 'Cairo')),
            ...
            Expanded(child: ElevatedButton.icon(
              ...
              label: Text(t('تحليل AI', 'AI Analysis'), style: const TextStyle(fontFamily: 'Cairo', color: Colors.white)),
  ...
  Widget _row(String label, String val) => Padding(
    ...
    child: Row(crossAxisAlignment: CrossAxisAlignment.start, children: [
      Text('$label: ', style: const TextStyle(fontFamily: 'Cairo', fontWeight: FontWeight.w700, fontSize: 12)),
      Expanded(child: Text(val, style: const TextStyle(fontFamily: 'Cairo', fontSize: 12, color: AppColors.lightMuted))),
    ]),
  );
    ...
    showModalBottomSheet(
      ...
      builder: (_) => Column(children: [
        Padding(
          ...
          child: Text(isAr ? 'سجل الماسحات' : 'Scan History',
              style: const TextStyle(fontFamily: 'Cairo', fontWeight: FontWeight.w700, fontSize: 16)),
        ),
        if (history.isEmpty)
          Expanded(child: Center(child: Text(isAr ? 'لا توجد ماسحات بعد' : 'No scans yet',
              style: const TextStyle(fontFamily: 'Cairo', color: AppColors.lightMuted))))
        else
          Expanded(child: ListView(children: history.map((r) => ListTile(
            leading: Text(_statusEmoji(r.status), style: const TextStyle(fontSize: 22)),
            title: Text(r.name, style: const TextStyle(fontFamily: 'Cairo', fontWeight: FontWeight.w600, fontSize: 13)),
            subtitle: Text(r.brand ?? '', style: const TextStyle(fontFamily: 'Cairo', fontSize: 11)),
            trailing: Text(
              isAr ? _labelAr(r.status) : _labelEn(r.status),
