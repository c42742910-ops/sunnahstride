...
class _SettingsState extends ConsumerState<SettingsScreen> {
  ...
  Future<void> _loadNotifPrefs() async {
    ...
    if (!mounted) return;
    setState(() {
      _notifWater   = p.getBool('notif_water')   ?? true;
      _notifWorkout = p.getBool('notif_workout')  ?? true;
      _notifMeal    = p.getBool('notif_meal')     ?? true;
    });
  }
  ...
  Widget build(BuildContext context) {
    ...
    Widget section(String label) => Padding(
      ...
      child: Text(label, style: const TextStyle(
          ...
          color: AppColors.sunnahGreen, letterSpacing: 0.5)),
    );
    ...
    Widget tile({
      ...
      child: ListTile(
        ...
        subtitle: subtitle != null
            ? Text(subtitle, style: TextStyle(fontFamily: 'Cairo', fontSize: 11, color: muted))
            : null,
        trailing: trailing,
        onTap: onTap,
    ...
    return Directionality(
      ...
      child: Scaffold(
        backgroundColor: bg,
        appBar: AppBar(
          title: Text(t('الإعدادات ⚙️', 'Settings ⚙️'),
        ...
        body: ListView(padding: const EdgeInsets.symmetric(horizontal: 16, vertical: 8),
          children: [
            ...
            tog(
              ...
              subtitle: isDark ? t('تبديل للضوء', 'Switch to light') : t('تبديل للظلام', 'Switch to dark'),
              value: isDark,
            ...
            tile(
              ...
              trailing: Row(mainAxisSize: MainAxisSize.min, children: [
                _langBtn(context, isAr, true, 'ع', 'ar'),
                const SizedBox(width: 6),
                _langBtn(context, isAr, false, 'EN', 'en'),
              ]),
            ),
            ...
            if (notifsOn) ...[
              ...
              tog(
                ...
                subtitle: t('يومياً في الصباح', 'Daily morning'),
                value: _notifWorkout,
                onChanged: (v) {
                  setState(() => _notifWorkout = v);
                  _saveNotifPref('notif_workout', v);
            ...
            tile(
              ...
              subtitle: '${ref.read(waterProvider).goal} ${t("كوب", "cups")}',
            ...
            tile(
              ...
              subtitle: '${ref.read(sleepProvider).goal.toStringAsFixed(1)} ${t("ساعة", "hrs")}',
              trailing: const Icon(Icons.chevron_right),
              onTap: () => _editSleepGoal(context, isAr),
            ),

            // ── SUBSCRIPTION ──────────────────────────────────
            section(t('الاشتراك ⭐', 'SUBSCRIPTION ⭐')),
            if (!isPrem)
              tile(
                emoji: '🔓',
                title: t('ترقية للبريميوم', 'Upgrade to Premium'),
                subtitle: t('افتح 10+ تمارين متقدمة وتحليل AI بلا حدود',
                    'Unlock 10+ advanced workouts & unlimited AI analysis'),
                trailing: Container(
              ...
              tile(
                emoji: '⭐',
                title: t('عضو بريميوم', 'Premium Member'),
                subtitle: t('شكراً لدعمك! — كل الميزات مفتوحة',
                    'Thank you! — All features unlocked'),
                titleColor: AppColors.barakahGold,
                trailing: const Icon(Icons.check_circle, color: AppColors.halalGreen),
              ),

            // ── DATA ──────────────────────────────────────────
            section(t('البيانات 🗂️', 'DATA 🗂️')),
            tile(
              emoji: '✏️',
            ...
            tile(
              ...
              subtitle: t('يساعدنا تقييم 5 نجوم كثيراً ❤️',
                  'A 5-star review helps us a lot ❤️'),
              trailing: const Icon(Icons.open_in_new, size: 16),
            ),
  ...
  void _editWaterGoal(BuildContext context, bool isAr) {
    final ctrl = TextEditingController(
        text: '${ref.read(waterProvider).goal}');
    showDialog(context: context, builder: (_) => AlertDialog(
      title: Text(isAr ? 'هدف الماء اليومي' : 'Daily Water Goal',
          style: const TextStyle(fontFamily: 'Cairo')),
      content: TextField(
        controller: ctrl, keyboardType: TextInputType.number,
        decoration: InputDecoration(
          hintText: isAr ? 'عدد الأكواب' : 'Number of cups',
          suffixText: isAr ? 'كوب' : 'cups',
        ),
        autofocus: true,
      ),
      actions: [
  ...
  void _editSleepGoal(BuildContext context, bool isAr) {
    ...
    showDialog(context: context, builder: (_) => AlertDialog(
      title: Text(isAr ? 'هدف النوم' : 'Sleep Goal',
          style: const TextStyle(fontFamily: 'Cairo')),
      content: TextField(
        controller: ctrl,
        keyboardType: const TextInputType.numberWithOptions(decimal: true),
        decoration: InputDecoration(
          hintText: isAr ? 'عدد الساعات' : 'Number of hours',
          suffixText: isAr ? 'ساعة' : 'hrs',
        ),
        autofocus: true,
      ),
      actions: [
        ...
        ElevatedButton(
          onPressed: () {
            final h = double.tryParse(ctrl.text.trim().replaceAll(',', '.')) ?? 8.0;
  ...
  void _confirmClearDay(BuildContext context, bool isAr) {
    showDialog(context: context, builder: (_) => AlertDialog(
      title: Text(isAr ? 'مسح سجل اليوم؟' : 'Clear Today\'s Data?',
          style: const TextStyle(fontFamily: 'Cairo', fontWeight: FontWeight.w700)),
      content: Text(
        isAr
            ? 'سيُمسح سجل الوجبات والخطوات والماء لليوم فقط. لا يمكن التراجع.'
            : 'Today\'s meals, steps, and water will be cleared. Cannot be undone.',
          ...
          onPressed: () async {
            ...
            if (context.mounted) {
              ScaffoldMessenger.of(context).showSnackBar(SnackBar(
                content: Text(isAr ? '✅ تم مسح سجل اليوم' : '✅ Today\'s log cleared',
                    style: const TextStyle(fontFamily: 'Cairo')),
