...
class _NutritionState extends ConsumerState<NutritionScreen>
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        title: Text(t('التغذية 🌿', 'Nutrition 🌿')),
        bottom: TabBar(
          controller: _tab,
          ...
          tabs: [
            Tab(text: t('السعرات', 'Calories')),
            Tab(text: t('الوصفات', 'Recipes')),
            Tab(text: t('مخطط AI', 'AI Planner')),
          ],
        ),
      ),
      body: TabBarView(
  ...
  Widget _buildCalories(bool isAr, bool isDark, dynamic profile) {
    ...
    return ListView(padding: const EdgeInsets.all(14), children: [
          ...
          SizedBox(
            ...
            child: Stack(alignment: Alignment.center, children: [
              ...
              Column(mainAxisAlignment: MainAxisAlignment.center, children: [
                Text('${cals.total}', style: TextStyle(fontFamily: 'Cairo', fontSize: 36, fontWeight: FontWeight.w900, color: calCol)),
                Text(t('مُستهلك', 'consumed'), style: TextStyle(fontFamily: 'Cairo', fontSize: 11, color: muted)),
              ]),
            ]),
          ),
          const SizedBox(height: 16),
          Row(mainAxisAlignment: MainAxisAlignment.spaceAround, children: [
            _statPill(t('مُستهلك','Consumed'), '${cals.total}', calCol),
            _statPill(t('متبقي','Remaining'), '${cals.remaining}', AppColors.halalGreen),
            _statPill(t('الهدف','Goal'), '${cals.goal}', AppColors.sunnahGreen),
          ]),
          const SizedBox(height: 14),
          // Macro bars from profile
          if (profile != null) ...[
            _macroBar(t('🍚 كربوهيدرات','🍚 Carbs'), profile.carbsGrams.toInt(), (profile.calorieGoalKcal / 4).toInt(), AppColors.waterBlue),
            const SizedBox(height: 6),
            _macroBar(t('🥩 بروتين','🥩 Protein'), profile.proteinGrams.toInt(), (profile.proteinGrams * 1.3).toInt(), AppColors.halalGreen),
            const SizedBox(height: 6),
            _macroBar(t('🧈 دهون','🧈 Fat'), profile.fatGrams.toInt(), (profile.calorieGoalKcal / 9 * 0.45).toInt(), AppColors.barakahGold),
            const SizedBox(height: 14),
          ],
          // Edit goal row
          if (_editingGoal) Row(children: [
            Expanded(child: TextField(
              controller: _goalCtrl..text = '${cals.goal}',
              keyboardType: TextInputType.number,
              textDirection: TextDirection.ltr,
              decoration: InputDecoration(
                labelText: t('الهدف اليومي (سعرة)','Daily Goal (kcal)'),
            ...
            child: Container(
              ...
              child: Row(mainAxisAlignment: MainAxisAlignment.center, children: [
                ...
                Text(t('✏️ تعديل الهدف','✏️ Edit Goal'),
      ...
      SizedBox(width: double.infinity, child: ElevatedButton.icon(
        ...
        label: Text(t('+ أضف وجبة','+ Add Meal'), style: const TextStyle(fontFamily: 'Cairo', fontSize: 14, color: Colors.white)),
      ...
      if (cals.entries.isEmpty)
        Center(child: Padding(padding: const EdgeInsets.all(20),
          child: Column(children: [
            ...
            Text(t('لم تُسجّل أي وجبات اليوم','No meals logged today'),
                style: TextStyle(fontFamily: 'Cairo', fontSize: 13, color: muted)),
          ]),
        ))
      else ...[
        Text(t('وجبات اليوم','Today\'s Meals'),
        ...
        ...cals.entries.map((e) => Container(
          ...
          child: Row(children: [
            ...
            Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              Text(e.name, style: const TextStyle(fontFamily: 'Cairo', fontWeight: FontWeight.w700, fontSize: 13)),
              Text('${_timeStr(e.time)}', style: TextStyle(fontFamily: 'Cairo', fontSize: 10, color: muted)),
            ])),
          ...
          child: Column(mainAxisSize: MainAxisSize.min, crossAxisAlignment: CrossAxisAlignment.start, children: [
            ...
            Wrap(spacing: 8, runSpacing: 8, children: kQuickFoods.map((f) => GestureDetector(
              ...
              child: Container(
                ...
                child: Text('${f.name}  ${f.kcal}kcal',
                    style: const TextStyle(fontFamily: 'Cairo', fontSize: 11, fontWeight: FontWeight.w700, color: AppColors.sunnahGreen)),
              ),
            )).toList()),
              ])),
            const Divider(height: 24),
            Text(t('إدخال مخصص','Custom Entry'),
                style: const TextStyle(fontFamily: 'Cairo', fontWeight: FontWeight.w700, fontSize: 14)),
            const SizedBox(height: 10),
            TextField(
              controller: _nameCtrl, textDirection: isAr ? TextDirection.rtl : TextDirection.ltr,
              decoration: InputDecoration(labelText: t('اسم الطعام','Food Name'), border: const OutlineInputBorder()),
            ),
            const SizedBox(height: 8),
            TextField(
              controller: _kcalCtrl, keyboardType: TextInputType.number, textDirection: TextDirection.ltr,
              decoration: InputDecoration(labelText: t('السعرات الحرارية','Calories (kcal)'), border: const OutlineInputBorder()),
            ),
            const SizedBox(height: 8),
            Row(children: [
              Expanded(child: TextField(
                controller: _proteinCtrl, keyboardType: TextInputType.number, textDirection: TextDirection.ltr,
                decoration: InputDecoration(labelText: 'P (g)', border: const OutlineInputBorder()),
              )),
              const SizedBox(width: 6),
              Expanded(child: TextField(
                controller: _carbsCtrl, keyboardType: TextInputType.number, textDirection: TextDirection.ltr,
                decoration: InputDecoration(labelText: 'C (g)', border: const OutlineInputBorder()),
              )),
              const SizedBox(width: 6),
              Expanded(child: TextField(
                controller: _fatCtrl, keyboardType: TextInputType.number, textDirection: TextDirection.ltr,
                decoration: InputDecoration(labelText: 'F (g)', border: const OutlineInputBorder()),
              )),
            ]),
            const SizedBox(height: 4),
            Text(t('بروتين / كربوهيدرات / دهون (اختياري)', 'Protein / Carbs / Fat (optional)'),
            ...
            SizedBox(width: double.infinity, child: ElevatedButton(
              onPressed: () async {
                ...
                if (name.isNotEmpty && kcal != null && kcal > 0) {
                  ...
                  final ft = double.tryParse(_fatCtrl.text) ?? 0;
                  await ref.read(caloriesProvider.notifier).addEntry(name, kcal, proteinG: p, carbsG: c, fatG: ft);
                  _nameCtrl.clear(); _kcalCtrl.clear();
              ...
              child: Text(t('✓ أضف للعداد','✓ Add to Tracker'),
  ...
  Widget _buildRecipes(bool isAr, bool isDark) {
    ...
    String t(String ar, String en) => isAr ? ar : en;
    return ListView(padding: const EdgeInsets.all(14), children: [
      Text(t('وصفات سنية 🌿','Sunnah Recipes 🌿'),
          style: TextStyle(fontFamily: 'Cairo', fontSize: 16, fontWeight: FontWeight.w700,
            color: isDark ? AppColors.darkText : AppColors.lightText)),
      const SizedBox(height: 4),
      Text(t('مبنية على سنة النبي ﷺ','Based on the Prophet\'s ﷺ Sunnah'),
      ...
      return Container(
        ...
        child: Padding(padding: const EdgeInsets.all(14), child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          GestureDetector(
            ...
            child: Row(children: [
              Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                Text(r.nameAr, style: const TextStyle(fontFamily: 'Cairo', fontWeight: FontWeight.w700, fontSize: 13)),
                const SizedBox(height: 4),
                Wrap(spacing: 6, runSpacing: 4, children: [
                  _chip('⏱️ ${r.timeMins}${t("د","m")}', AppColors.waterBlue),
                  _chip('💰 ${r.costEGP} EGP', AppColors.barakahGold),
                  _chip('🔥 ${r.kcal} ${t("ك.ح","kcal")}', AppColors.haramRed),
                ]),
                const SizedBox(height: 6),
                Wrap(spacing: 6, runSpacing: 4, children: r.sunnahIngredients.map((s) =>
                  _chip('🌿 $s', AppColors.halalGreen)).toList()),
              ])),
          ...
          AnimatedSize(duration: const Duration(milliseconds: 300), curve: Curves.easeInOut,
            child: isOpen ? Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              ...
              Text(t('المكونات:','Ingredients:'),
                  style: const TextStyle(fontFamily: 'Cairo', fontWeight: FontWeight.w700, fontSize: 12, color: AppColors.sunnahGreen)),
              ...r.ingredients.map((ing) => Padding(padding: const EdgeInsets.only(top: 3, right: 10, left: 10),
                child: Text('• $ing', style: const TextStyle(fontFamily: 'Cairo', fontSize: 12)))),
              const SizedBox(height: 10),
              Text(t('طريقة التحضير:','Steps:'),
                  style: const TextStyle(fontFamily: 'Cairo', fontWeight: FontWeight.w700, fontSize: 12, color: AppColors.sunnahGreen)),
              ...r.steps.asMap().entries.map((e) => Padding(padding: const EdgeInsets.only(top: 4, right: 10, left: 10),
                child: Text('${e.key + 1}. ${e.value}', style: const TextStyle(fontFamily: 'Cairo', fontSize: 12, height: 1.5)))),
              const SizedBox(height: 12),
              SizedBox(width: double.infinity, child: OutlinedButton(
                onPressed: () {
                  ...
                  ScaffoldMessenger.of(context).showSnackBar(SnackBar(
                    content: Text(isAr ? '✓ تمت الإضافة' : '✓ Added to tracker',
                        style: const TextStyle(fontFamily: 'Cairo')),
                    backgroundColor: AppColors.sunnahGreen,
                    duration: const Duration(seconds: 2),
                  ));
                },
                style: OutlinedButton.styleFrom(foregroundColor: AppColors.sunnahGreen, side: const BorderSide(color: AppColors.sunnahGreen)),
                child: Text(t('+ أضف للعداد','+ Add to Tracker'), style: const TextStyle(fontFamily: 'Cairo')),
              )),
  ...
  Widget _buildAIPlanner(bool isAr, bool isDark, dynamic profile) {
    ...
    if (!isPremium) {
      return Center(child: Padding(padding: const EdgeInsets.all(28), child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [
        ...
        const SizedBox(height: 16),
        Text(t('مخطط الوجبات بالذكاء الاصطناعي','AI Meal Planner'),
            textAlign: TextAlign.center,
            style: const TextStyle(fontFamily: 'Cairo', fontSize: 22, fontWeight: FontWeight.w800)),
        const SizedBox(height: 8),
        Text(t('يُخصّص خطة وجبات كاملة بناءً على جسمك وأهدافك وتفضيلاتك الحلالية',
              'Creates a complete meal plan based on your body, goals & halal preferences'),
            textAlign: TextAlign.center,
        ...
        SizedBox(width: double.infinity, child: ElevatedButton(
          ...
          child: Text(t('🔓 افتح بريميوم','🔓 Unlock Premium'), style: const TextStyle(fontFamily: 'Cairo', fontSize: 16, color: Colors.white, fontWeight: FontWeight.w700)),
        )),
      ])));
    }
    ...
    return ListView(padding: const EdgeInsets.all(14), children: [
      ...
      Container(
        ...
        child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          Text(t('اطلب خطة وجبات مخصصة:','Request a Personalized Meal Plan:'),
          ...
          TextField(
            ...
            decoration: InputDecoration(
              hintText: isAr
                ? 'مثال: أريد خطة ٣ وجبات لخسارة الوزن مع تجنب الأرز...'
                : 'e.g. I want a 3-meal plan for weight loss avoiding rice...',
          ...
          SizedBox(width: double.infinity, child: ElevatedButton(
            ...
            child: _aiLoading
              ...
              : Text(t('🤖 ولّد الخطة','🤖 Generate Plan'), style: const TextStyle(fontFamily: 'Cairo', color: Colors.white, fontSize: 13)),
          )),
        ]),
      ),
      if (_aiResult != null) ...[
  ...
  Widget _buildAIResult(bool isAr, bool isDark, Color bg, Color muted) {
    ...
    // If real AI response, show it directly
    final isRealAI = _aiResult != null && _aiResult != 'fallback';
    // Show error state for fallback
    if (_aiResult == 'fallback') {
      return Container(
        ...
        child: Column(children: [
          ...
          const SizedBox(height: 8),
          Text(isAr
            ? 'تعذّر الاتصال بـ AI. تأكد من اتصالك بالإنترنت وحاول مجدداً.'
            : 'Could not reach AI. Check your internet connection and try again.',
            textAlign: TextAlign.center,
          ...
          TextButton(
            ...
            child: Text(isAr ? '↺ حاول مجدداً' : '↺ Try Again',
              style: const TextStyle(fontFamily: 'Cairo', color: AppColors.sunnahGreen)),
          ),
        ]),
      );
    }
      ...
      return Container(
        ...
        child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          Row(children: [
            ...
            Text(t('خطتك الشخصية من AI', 'Your AI Personalized Plan'),
                style: const TextStyle(fontFamily: 'Cairo', fontWeight: FontWeight.w700, fontSize: 15, color: AppColors.sunnahGreen)),
          ]),
          const SizedBox(height: 12),
          Text(_aiResult!, style: TextStyle(fontFamily: 'Cairo', fontSize: 13, height: 1.7,
              color: isDark ? AppColors.darkText : AppColors.lightText)),
          const SizedBox(height: 14),
          SizedBox(width: double.infinity, child: OutlinedButton(
            onPressed: () => setState(() { _aiResult = null; _aiCtrl.clear(); }),
            style: OutlinedButton.styleFrom(foregroundColor: AppColors.sunnahGreen, side: const BorderSide(color: AppColors.sunnahGreen)),
            child: Text(t('↺ طلب جديد', '↺ New Request'), style: const TextStyle(fontFamily: 'Cairo')),
          )),
        ]),
      );
    }
    ...
    return Container(
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        Text(t('✨ خطتك الشخصية','✨ Your Personalized Plan'),
            style: const TextStyle(fontFamily: 'Cairo', fontWeight: FontWeight.w700, fontSize: 15, color: AppColors.sunnahGreen)),
        const SizedBox(height: 14),
        _mealPlanRow('🌅', t('الفطور','Breakfast'),
          isAr ? 'شوفان بالعسل والتمر + كوب حليب\nبيضتان مسلوقتان + لبن' : 'Oats with honey & dates + milk\n2 boiled eggs + yogurt',
          bf, isAr),
        const SizedBox(height: 12),
        _mealPlanRow('☀️', t('الغداء','Lunch'),
          isAr ? 'دجاج مشوي حلال + أرز بني + سلطة خضراء\nزيت زيتون + ثوم + ليمون' : 'Halal grilled chicken + brown rice + green salad\nOlive oil + garlic + lemon',
          lGoal, isAr),
        const SizedBox(height: 12),
        _mealPlanRow('🌙', t('العشاء','Dinner'),
          isAr ? 'سمك مشوي + خضار مشكلة + عدس\nشوربة عدس بالليمون' : 'Grilled fish + mixed vegetables + lentils\nLentil soup with lemon',
          dGoal, isAr),
        const SizedBox(height: 16),
        Container(
          padding: const EdgeInsets.all(12),
          ...
          child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            Text(t('📖 توصيات سنية:','📖 Sunnah recommendations:'),
                style: const TextStyle(fontFamily: 'Cairo', fontWeight: FontWeight.w700, fontSize: 12, color: AppColors.barakahGold)),
            const SizedBox(height: 6),
            Text(
              isAr ? '• تمر وماء عند الإفطار\n• ٧ تمرات صباحاً للوقاية\n• لا تأكل حتى تجوع\n• الصيام الاثنين والخميس مفيد جداً'
                   : '• Dates & water for iftar\n• 7 dates in the morning for protection\n• Don\'t eat until you\'re hungry\n• Mon/Thu fasting is highly beneficial',
              style: const TextStyle(fontFamily: 'Cairo', fontSize: 11, height: 1.7, color: AppColors.lightMuted),
            ),
          ]),
        ),
      ]),
  ...
  Widget _mealPlanRow(String emoji, String meal, String desc, int kcal, bool isAr) {
    return Row(crossAxisAlignment: CrossAxisAlignment.start, children: [
      ...
      const SizedBox(width: 10),
      Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        Text(meal, style: const TextStyle(fontFamily: 'Cairo', fontWeight: FontWeight.w700, fontSize: 13)),
        Text(desc, style: const TextStyle(fontFamily: 'Cairo', fontSize: 11, height: 1.5, color: AppColors.lightMuted)),
      ])),
      Text('$kcal\nkcal', textAlign: TextAlign.center,
          style: const TextStyle(fontFamily: 'Cairo', fontSize: 11, fontWeight: FontWeight.w700, color: AppColors.halalGreen)),
    ]);
  }
  ...
  Widget _macroRingsCard(bool isAr, bool isDark, Color cardBg, Color muted, CaloriesState cals) {
    ...
    Widget ring(double val, double total, Color col, String label) {
      ...
      return Column(children: [
        SizedBox(width: 72, height: 72, child: Stack(alignment: Alignment.center, children: [
          ...
          Column(mainAxisAlignment: MainAxisAlignment.center, children: [
            Text('${val.toInt()}g', style: TextStyle(fontFamily: 'Cairo',
                fontSize: 11, fontWeight: FontWeight.w900, color: col)),
            Text('${(pct * 100).toInt()}%', style: TextStyle(
                fontFamily: 'Cairo', fontSize: 8, color: muted)),
          ]),
        ])),
        const SizedBox(height: 6),
        Text(label, style: TextStyle(fontFamily: 'Cairo', fontSize: 10, color: muted)),
      ]);
    }
    ...
    return Container(
      ...
      decoration: BoxDecoration(color: cardBg, borderRadius: BorderRadius.circular(18),
          boxShadow: [BoxShadow(color: Colors.black.withOpacity(0.06), blurRadius: 12)]),
      child: Column(children: [
        Text(t('💊 الماكروز المُتناولة اليوم', '💊 Today\'s Macros'),
            style: TextStyle(fontFamily: 'Cairo', fontWeight: FontWeight.w700,
                fontSize: 13, color: isDark ? AppColors.darkText : AppColors.lightText)),
        const SizedBox(height: 16),
        Row(mainAxisAlignment: MainAxisAlignment.spaceAround, children: [
          ring(cals.proteinTotal, protGoal, AppColors.halalGreen, t('بروتين', 'Protein')),
          ring(cals.carbsTotal, carbGoal, AppColors.waterBlue, t('كربوهيدرات', 'Carbs')),
          ring(cals.fatTotal, fatGoal, AppColors.barakahGold, t('دهون', 'Fat')),
        ]),
      ]),
    );
  }
    ...
    return weekly.when(
      ...
      data: (data) {
        ...
        final bars = List.generate(7, (i) {
          ...
          final found = data.where((d) => d['date'] == key).toList();
          final kcal  = found.isNotEmpty ? (found.first['kcal'] as int).toDouble() : 0.0;
        ...
        return Container(
          ...
          decoration: BoxDecoration(color: cardBg, borderRadius: BorderRadius.circular(18),
              boxShadow: [BoxShadow(color: Colors.black.withOpacity(0.06), blurRadius: 12)]),
          child: Column(children: [
            Text(t('📊 السعرات — آخر ٧ أيام', '📊 Calories — Last 7 Days'),
                style: TextStyle(fontFamily: 'Cairo', fontWeight: FontWeight.w700,
                    fontSize: 13, color: isDark ? AppColors.darkText : AppColors.lightText)),
            const SizedBox(height: 16),
            SizedBox(height: 120, child: BarChart(BarChartData(
              ...
              titlesData: FlTitlesData(
                bottomTitles: AxisTitles(sideTitles: SideTitles(
                  ...
                  getTitlesWidget: (v, _) => Text(dayNames[v.toInt() % 7],
  ...
  Widget _foodChip(QuickFood f, BuildContext context, bool isAr) {
    return GestureDetector(
      ...
      child: Container(
        ...
        child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          Text(f.name, style: const TextStyle(fontFamily: 'Cairo', fontSize: 11,
              fontWeight: FontWeight.w700, color: AppColors.sunnahGreen)),
          Text('${f.kcal} kcal  P:${f.proteinG.toInt()}g  C:${f.carbsG.toInt()}g  F:${f.fatG.toInt()}g',
  ...
  Widget _statPill(String label, String val, Color color) {
    return Column(children: [
      Text(val, style: TextStyle(fontFamily: 'Cairo', fontSize: 18, fontWeight: FontWeight.w900, color: color)),
  ...
  Widget _macroBar(String label, int val, int max, Color color) {
    return Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
      Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
        Text(label, style: const TextStyle(fontFamily: 'Cairo', fontSize: 12, fontWeight: FontWeight.w600)),
        Text('${val}g', style: TextStyle(fontFamily: 'Cairo', fontSize: 12, fontWeight: FontWeight.w900, color: color)),
      ]),
      const SizedBox(height: 4),
  ...
  Widget _chip(String label, Color color) => Container(
    ...
    child: Text(label, style: TextStyle(fontFamily: 'Cairo', fontSize: 10, fontWeight: FontWeight.w700, color: color)),
  );
