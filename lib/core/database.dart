...
class AppDatabase {
  ...
  static Future<void> _upgrade(Database db, int oldV, int newV) async {
    if (oldV < 2) {
      await db.execute('ALTER TABLE meal_entries ADD COLUMN protein_g REAL DEFAULT 0');
      await db.execute('ALTER TABLE meal_entries ADD COLUMN carbs_g REAL DEFAULT 0');
      await db.execute('ALTER TABLE meal_entries ADD COLUMN fat_g REAL DEFAULT 0');
    }
    if (oldV < 3) {
      try {
        await db.execute('''
  ...
  static Future<void> upsertSummary({
    ...
    if (existing == null) {
      ...
      if (updates.isNotEmpty) {
        await d.update('daily_summary', updates, where: 'date_key = ?', whereArgs: [key]);
      }
    }
  }
  ...
  static Future<int> getTodayWorkoutMinutes() async {
    final d = await db;
    final rows = await d.rawQuery(
      "SELECT SUM(minutes) as total FROM workout_log WHERE date_key = ?", [_today()]);
